...
void addElems(set<char> &a,set<char> &b){ //add elements of b to a
		...
		if(grammar[i].size()>0){
			for(int j=0;j<grammar[i].size();j++){
				...
				for(int k=0;k<prod.size();k++){
					if(prod[k] == x){
						if(k == prod.size()-1){ //the case when it is the last element of the rhs
							if(follow[i].size()==0)
								computeFollow(i);
							addElems(follow[v],follow[i]);
						}else{
							if(isupper(prod[k+1])){//case when the next element is a variable
								addElems(follow[v],first[prod[k+1]-'A']);
							}else{ //case when next element is a terminal
	...
	for(int i=0;i<26;i++){
		if(curr[i].size()>0){
			for(int j=0;j<curr[i].size();j++){
				string prod = curr[i][j],bd,ad;
				int pos = findDot(prod,bd,ad);
				if(pos == prod.size()-1){//in this case, action part of itemset must be filled with reduce
	...
	for(int i=0;i<table.size();i++){
		for(map<char,vector<string> >::iterator it=table[i].action.begin();it!=table[i].action.end();it++){
			if((it->second).size()==2){
				...
				if((it->first) == (it->second)[r][opPos]){//in this case, since operators are same, we have to resolve the associativity
					if(assoc){
						(it->second).erase((it->second).begin()+r);
					}else{
						(it->second).erase((it->second).begin()+s);
					}
				}else{//operators are not the same, so we have to resolve precedence
					if(prec){
